...
import BullaManagerArtifact from "../artifacts/contracts/BullaV1.sol/BullaManager.json";
...
import { payClaim, rejectClaim } from "./lib-dev/bulla-claim";
import { getBytes32FromMultihash, getMultihashFromBytes32 } from "./lib-dev/mutlihash";

chai.use(solidity);

const [ masterWallet, 
    wallet2, wallet3, creditor, 
    debtor, collectorWallet] = new MockProvider().getWallets()
...
describe("bulla network contract testing", () => {    
    ...
    describe("BullaManager checks", async () => {
        before(async () => {
            bullaManager = (await deployContract(
                masterWallet, BullaManagerArtifact, [toBytes32("MyTestCreator"), collectorWallet.address, 100 ]
            )) as BullaManager;                     
        });
        ...
        it("should revert because masterWallet is no longer the owner", async () => {
            await expect(setOwner(wallet2.address, bullaManager, masterWallet))
                .to.be.reverted;
        });
        ...
        it("should create bulla group and map to event logs", async () => {            
            const {receipt, newBullaGroupEvent} = await createBullaGroup(
                {...bullaGroupParams, signer:masterWallet, bullaManager:bullaManager});
    ...
    describe("bulla claim checks", async () => {
        it("should add ipsf hash", async () => {
            ...
            const bullaHash = await bullaClaim
                .connect(creditor)
                .multihash()
            console.log(bullaHash)
        ...
        it("should pay part bulla claim", async () => {            
            const {receipt, claimActionEvent, feePaidEvent} = await payClaim({
                bullaClaim:bullaClaim, paymentAmount:.5, signer:debtor
            });
        ...
        it("should create and reject new bulla claim", async () => {
            ...
            const tx = await rejectClaim({bullaClaim:bullaClaim2, signer:debtor})
