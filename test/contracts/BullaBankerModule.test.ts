...
describe('test module', async () => {
    let [safeOwner1, safeOwner2, outsider, collector, creditor] = declareSignerWithAddress();
    let feeBasisPoint = 1000;

    const setupTests = deployments.createFixture(async ({ deployments }) => {
        await deployments.fixture();
        [safeOwner1, safeOwner2, outsider, collector, creditor] = await ethers.getSigners();
        const ERC20 = (await hre.ethers.getContractFactory('BullaToken')) as BullaToken__factory;
        const BullaManager = (await hre.ethers.getContractFactory('BullaManager')) as BullaManager__factory;
        const BullaClaimERC721 = (await hre.ethers.getContractFactory('BullaClaimERC721')) as BullaClaimERC721__factory;
        const BullaBanker = (await hre.ethers.getContractFactory('BullaBanker')) as BullaBanker__factory;
        const BatchBulla = (await hre.ethers.getContractFactory('BatchCreate')) as BatchCreate__factory;
        const Safe = (await hre.ethers.getContractFactory('TestSafe')) as TestSafe__factory;
        const BullaBankerModule = (await hre.ethers.getContractFactory('BullaBankerModule')) as BullaBankerModule__factory;
        ...
        const bullaClaim = await BullaClaimERC721.deploy(bullaManager.address, 'ipfs.io/ipfs/');
        const bullaBanker = await BullaBanker.deploy(bullaClaim.address);
        const batchCreate = await BatchBulla.deploy(bullaBanker.address, bullaClaim.address, 20);
    ...
    describe('Bulla Banker - Gnosis Safe Module', async () => {
        describe('createClaim', async () => {
            it('should create a claim via module', async () => {
                const { bullaBankerModule, bullaToken, bullaClaim, safe } = await setupTests();
                const tokenId = '1';
        ...
        describe('batchCreate', async () => {
            it('should create a batchClaim via module', async () => {
                ...
                await expect(bullaBankerModule.batchCreate(batchPayment)).to.emit(bullaClaim, 'ClaimCreated');
            ...
            it('should revert if params are incorrect', async () => {
                ...
                await expect(bullaBankerModule.batchCreate(batchPayment)).to.be.revertedWith('BULLAMODULE: Batch create failed');
            });
            ...
            it('should revert if sender != safe owner', async () => {
                ...
                await expect(bullaBankerModule.connect(outsider).batchCreate(batchPayment)).to.be.revertedWith(
        ...
        describe('updateTag', async () => {
            it('should update a bullaTag via the safe', async () => {
                const { bullaBankerModule, bullaToken, safe, bullaBanker, bullaManager } = await setupTests();
                const tokenId = '1';
                ...
                await expect(bullaBankerModule.connect(safeOwner1).updateBullaTag(tokenId, tag))
                    .to.emit(bullaBanker, 'BullaTagUpdated')
                    .withArgs(bullaManager.address, tokenId, safe.address, tag, (await ethers.provider.getBlock('latest')).timestamp + 1);
            });
            ...
            it('should revert if sender != safe owner', async () => {
                ...
                await expect(
                    bullaBankerModule.connect(outsider).updateBullaTag(tokenId, utils.formatBytes32String('outsider tag 😈')),
                ).to.be.revertedWith('BULLAMODULE: Not safe owner');
            });
        });
        ...
        describe('rejectClaim', async () => {
            it('reject inbound claims', async () => {
                const { bullaBankerModule, bullaToken, safe, bullaClaim, bullaBanker } = await setupTests();
                const tokenId = '1';
                ...
                await expect(bullaBankerModule.connect(safeOwner1).rejectClaim(tokenId)).to.emit(bullaClaim, 'ClaimRejected');
            });

            it('should revert if sender != safe owner', async () => {
                const { bullaBankerModule, bullaToken, safe, bullaBanker } = await setupTests();
                const tokenId = '1';
                ...
                await expect(bullaBankerModule.connect(outsider).rejectClaim(tokenId)).to.be.revertedWith('BULLAMODULE: Not safe owner');
            });
        });
        ...
        describe('rescind claim', async () => {
            it('should rescind a outbound claim', async () => {
                ...
                expect(bullaBankerModule.rescindClaim(tokenId)).to.emit(bullaClaim, 'ClaimRescinded');
            });
            ...
            it('should revert if sender != safe owner', async () => {
                ...
                await expect(bullaBankerModule.connect(outsider).rescindClaim(tokenId)).to.be.revertedWith('BULLAMODULE: Not safe owner');
            });
        });
    });
});
