...
//     const [collector, newWallet, owner, member, nonMember] = new MockProvider().getWallets();
...
//             expect(await bullaGroup.groupType()).to.equal(ethers.utils.formatBytes32String("BullaGroup Type"));
//         });
...
//             await expect(bullaGroup.joinGroup().then(tx => tx.wait())).to.be.revertedWith(
//                 "members cannot join a group"
//             );
//         });
...
//         });
//         it("should emit  Membership event", async function () {
//             expect(await bullaGroup.connect(member).leaveGroup()).to.emit(bullaGroup, "Membership");
//         });
...
//             expect(await bullaGroup.createBulla("bulla description", 1000)).to.emit(bullaGroup, "NewBulla");
//         });
...
//                 .createBullaClaim(100, creditor.address, debtor.address, "BullaClaim description", 60 * 1000)
//                 .then(tx => tx.wait());
