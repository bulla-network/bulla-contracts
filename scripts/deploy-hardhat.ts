...
const hardhatDeploy = async () => {
    ...
    const { address: managerAddress } = await deploy('BullaManager', {
        from: deployer,
        args: [ethers.utils.formatBytes32String('BullaManager v1'), '0x89e03E7980C92fd81Ed3A9b72F5c73fDf57E5e6D', 0],
        log: true,
    });
    ...
    const { address: wethAddress } = await WETH.deploy(utils.parseEther('10'), 'Wrapped Ether', BigNumber.from(18), 'WETH');
