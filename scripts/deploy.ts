...
const deployCreator = async function () {
    ...
    const { address, receipt } = await deploy("JournalGroupCreator", {
        from: deployer,
        args: [ethers.utils.formatBytes32String("from hardhat deploy"),
    ...
    const creatorContract = new Contract(address,CreatorArtifact.abi) as JournalGroupCreator
    
    const pbReceipt = await creatorContract.connect(signer)
      .createJournalGroup("personal banker token", toBytes32('pb'), false)
      .then(tx=>tx.wait())

    const pbAddress : string = pbReceipt.events && pbReceipt.events[0].args?.groupAddress || "failed" 
